...
SECRET_PATH="${INFISICAL_SECRET_PATH:-/loket-kemenag}"
...
if [ -n "$TOKEN" ]; then
    ...
    echo "[Infisical] Menginjeksi secrets dari $API_DOMAIN (env: $ENV_TARGET, path: $SECRET_PATH)..."
    exec infisical run --token="$TOKEN" --domain="$API_DOMAIN" --env="$ENV_TARGET" $PROJECT_ARG --silent --path="$SECRET_PATH" -- "$@"
else
    echo "[Infisical] Token tidak ditemukan, menjalankan aplikasi langsung..."
    exec "$@"
fi
